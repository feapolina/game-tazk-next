...
const getPlatformColor = (platform: string = "") => {
  ...
  if (p.includes("playstation"))
    return "bg-indigo-600 hover:bg-indigo-700 text-white border-none";
  if (p.includes("xbox"))
  ...
  if (p.includes("mobile"))
    return "bg-pink-600 hover:bg-pink-700 text-white border-none";
...
export const GameCard = forwardRef<HTMLDivElement, GameCardProps>(
  (
    ...
    const baseClasses = `relative rounded-lg shadow-md group hover:shadow-xl transition-all duration-300 cursor-pointer ${touchClass}`;
    ...
    return (
      ...
      >
        <div className="relative w-full h-full bg-neutral-900 rounded-[18px] overflow-hidden">
          <div
            onClick={() => onClick?.(game)}
            className="w-full h-full relative group/content"
          >
            <img
              src={optimizeCoverUrl(game.cover_url)}
              alt={game.name}
